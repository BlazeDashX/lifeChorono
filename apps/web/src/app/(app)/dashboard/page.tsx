...
const fetchDashboard = async (date: string) => {
  console.log('Fetching dashboard for date:', date);
  try {
    const url = `/dashboard/week?weekStart=${date}`;
    console.log('Full URL being called:', `http://localhost:3003${url}`);
...
export default function DashboardPage() {
  ...
  console.log('Dashboard page rendering, dateStr:', dateStr);
  ...
  if (error) {
    ...
    return (
      <AppLayout>
        <div className="p-8 text-red-500">Error loading dashboard: {error.message}</div>
      </AppLayout>
    );
  }
