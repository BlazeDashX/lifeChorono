...
import { Plus, ChevronLeft, ChevronRight, Pencil, Trash2, Check, X, Zap, Waves, Moon, Minus, Sparkles } from 'lucide-react';
...
// ── Ghost visual weight by confidence ─────────────────────────────────────────
function gStyle(conf:number) {
...
function GhostBlock({ghost,dateStr,onDone}:{ghost:any;dateStr:string;onDone:()=>void}) {
  ...
  const st=gStyle(ghost.confidence);
  const {Icon}=c;
...
function EmptyCanvas({isToday}:{isToday:boolean}) {
  return (
    <div className="absolute inset-x-0 top-36 flex flex-col items-center gap-3 text-center pointer-events-none">
      ...
      <p className="text-xs max-w-[180px] leading-relaxed" style={{color:'#2A2A3A'}}>{isToday?'Tap + to begin recording your day.':'Nothing was recorded on this day.'}</p>
    </div>
  );
}
...
export default function LogPage() {
  ...
  useEffect(()=>{
    ...
    if (today) { const n=new Date(),off=n.getHours()*60+n.getMinutes(); setTimeout(()=>ref.current?.scrollTo({top:Math.max(0,off-160),behavior:'smooth'}),120); }
  ...
  const {data:ghosts=[]}=useQuery({queryKey:['ghosts',ds],queryFn:()=>api.get(`/schedule-templates/ghosts?date=${ds}`).then(r=>r.data),enabled:today,staleTime:5*60_000});
  const {data:adaptations=[]}=useQuery({queryKey:['adaptations'],queryFn:()=>api.get('/schedule-templates/adaptations').then(r=>r.data),staleTime:60_000});
        ...
        <div className="flex-shrink-0 px-5 pt-4 pb-3">
          <div className="relative flex items-center justify-center">
            ...
            <div className="text-center">
              <h1 className="text-base font-semibold tracking-tight" style={{color:'#F1F0FF'}}>{today?`Today — ${format(date,'MMMM d, yyyy')}`:format(date,'MMMM d, yyyy')}</h1>
          ...
          {!today&&<div className="flex justify-center mt-2"><span className="text-xs px-2.5 py-0.5 rounded-full" style={{backgroundColor:'rgba(74,74,106,0.1)',color:'#3A3A5A',border:'1px solid #141428'}}>Archived view</span></div>}
          <div className="mt-3 h-px" style={{background:'linear-gradient(90deg,transparent,#141428 20%,#141428 80%,transparent)'}}/>
        </div>

        {/* Adaptation banners — only today, max 2 */}
        {today&&(adaptations as any[]).length>0&&(
          <div className="flex-shrink-0 px-5 pb-1">
            {(adaptations as any[]).slice(0,2).map((s:any)=>(<AdaptationBanner key={`${s.blockId}-${s.dayOfWeek}`} s={s}/>))}
          </div>
        )}
        ...
        <div className="flex-1 flex overflow-hidden" style={{maxWidth:'960px',margin:'0 auto',width:'100%',padding:'0 16px 16px'}}>
          ...
          <div ref={ref} className="flex-1 overflow-y-auto min-w-0" style={{scrollbarWidth:'thin',scrollbarColor:'#111120 transparent'}}>
            <div className="relative" style={{height:'1440px',paddingLeft:'52px',paddingRight:'4px'}}>
              {[...Array(24)].map((_,i)=>{ const is6=i%6===0,is3=i%3===0&&!is6; return (
              ...
              {today&&<NowLine/>}
              {!(entries as any[]).length&&!pendingGhosts.length&&<EmptyCanvas isToday={today}/>}

              {/* Entries container — ghosts behind real entries */}
              <div className="absolute top-0 bottom-0" style={{left:'56px',right:0}}>
                {pendingGhosts.map((g:any)=>(
                  <GhostBlock key={g.id} ghost={g} dateStr={ds} onDone={()=>{ qc.invalidateQueries({queryKey:['ghosts',ds]}); qc.invalidateQueries({queryKey:['entries',ds]}); }}/>
                ))}
                {(entries as any[]).map((e,i)=>(
                  <EntryBlock key={e.id} entry={e} dateStr={ds} isEditing={editId===e.id} onEdit={()=>setEditId(e.id)} onDelete={()=>remove(e.id,e.title)} onSave={f=>save(e.id,f)} onCancel={()=>setEditId(null)} isSaving={upding} index={i}/>
                ))}
              </div>
          ...
          <div className="flex-shrink-0 overflow-hidden pt-1" style={{width:'192px'}}>
            <RightPanel dateStr={ds} onSelect={(task:any)=>{ setPrefill(task); setOpen(true); }}/>
          </div>
        </div>

        <button onClick={()=>{ setPrefill(null); setOpen(true); }} className="fixed z-20 w-11 h-11 rounded-full flex items-center justify-center transition-all duration-200 hover:scale-110 active:scale-95" style={{bottom:'24px',right:'28px',background:'linear-gradient(135deg,#7C3AED,#6D28D9)',boxShadow:'0 0 20px rgba(124,58,237,0.5),0 0 40px rgba(124,58,237,0.15)'}} onMouseEnter={e=>((e.currentTarget as HTMLElement).style.boxShadow='0 0 28px rgba(124,58,237,0.7),0 0 56px rgba(124,58,237,0.25)')} onMouseLeave={e=>((e.currentTarget as HTMLElement).style.boxShadow='0 0 20px rgba(124,58,237,0.5),0 0 40px rgba(124,58,237,0.15)')}>
